const saleDOM = document.getElementById("salesContainer");
...
salesByWeek.forEach(sale => {
  ...
  saleDOM.innerHTML += `<h2>${sale[`sales_agent`][`first_name`]} ${sale[`sales_agent`][`last_name`]}</h2>`
  for (const key of keys) {
    saleDOM.innerHTML += `<div>${key}: ${sale.vehicle[key]}</div>`;
  }
  saleDOM.innerHTML += `<span><h2>Profit: $${sale[`gross_profit`]}</h2></span>`
});
